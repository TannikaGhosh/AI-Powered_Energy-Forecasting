...
try:
    ...
    df_3d = pd.read_csv('data/raw/advanced_energy_data.csv', index_col='Datetime', parse_dates=True)
except FileNotFoundError:
    print("Please ensure both 'appliance_data.csv' and 'advanced_energy_data.csv' exist.")

# Load hardware-safe Scikit-Learn Multilayer Perceptron
...
# --- Risk features setup for 3D tab ---
try:
    df_3d['hour'] = df_3d.index.hour
    df_3d['day_of_week'] = df_3d.index.dayofweek
    df_3d['day_name'] = df_3d.index.day_name()
    df_3d['date'] = df_3d.index.date
    df_3d['composite_risk'] = (df_3d['PowerCut_Risk'] + df_3d['ShortCircuit_Risk'] + df_3d['ExcessElectricity_Risk']) / 3
except Exception as e:
    print(f"Error structuring 3D dataset: {e}")

# --- ML Predition implementation ---
...
def get_shedding_plan(current_loads_dict, predicted_load, threshold_watts=4500):
    if predicted_load <= threshold_watts:
        return {}, f"✅ Predicted load ({predicted_load:.0f}W) within threshold."
    excess = predicted_load - threshold_watts
    to_shed = []
    for app in PRIORITY_ORDER:
        if current_loads_dict.get(app, 0) > 0:
            to_shed.append(app)
            excess -= current_loads_dict[app]
            if excess <= 0:
                break
    if not to_shed:
        return {}, f"⚠️ CRITICAL: Predicted {predicted_load:.0f}W > {threshold_watts}W but no shedable appliances."
    plan = {app: 0 for app in to_shed}
    msg = f"🔌 Shedding plan: Turn OFF {', '.join([APPLIANCE_NAMES[a] for a in to_shed])}."
    return plan, msg

# --- Sector Carbon Credit Analysis Setup ---
sector_names = [s.replace('_', ' ') for s in SECTORS.keys()]
sector_keys = list(SECTORS.keys())

all_sector_data = {}
sector_carbon_results = {}
for key in sector_keys:
    df_sector = generate_sector_data(key, days=365)
    all_sector_data[key] = df_sector
    credits, co2 = calculate_carbon_credits(df_sector['Energy_kW'],
                                            SECTORS[key]['baseline_waste'],
                                            SECTORS[key]['carbon_intensity'])
    sector_carbon_results[key] = {'credits': credits, 'co2_tons': co2}

# --- Initialize App ---
app = dash.Dash(__name__)
app.title = "Energy Sector Analysis: Carbon Credits in ₹"

app.layout = html.Div([
    html.H1("⚡ Energy Sector Analysis: Carbon Credits in ₹", style={'textAlign': 'center'}),
    html.H3("AI Forecasting + Load Shedding + Sector Carbon Credit Comparison", style={'textAlign': 'center', 'color': '#555'}),
    dcc.Tabs(id='tabs', value='tab-forecast', children=[
        dcc.Tab(label='📈 Neural Network Forecast & Load Shedding', value='tab-forecast'),
        dcc.Tab(label='🌍 Energy Analytics', value='tab-3d'),
        dcc.Tab(label='🏭 Sector Carbon Credits', value='tab-sector'),
    ]),
...
forecast_layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Slider(id='threshold-slider', min=100, max=7000, step=100, value=4500,
...
analytics_3d_layout = html.Div([
    html.Div([
        html.Label("Select Date Range:"),
        dcc.DatePickerRange(
            id='date-range',
            start_date=df_3d.index.min().date(),
            end_date=df_3d.index.max().date(),
...
sector_layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Dropdown(
            id='sector-dropdown',
            options=[{'label': s, 'value': k} for s, k in zip(sector_names, sector_keys)],
            value='Domestic',
            clearable=False,
...
def update_sector_analysis(sector_key):
    df_sector = all_sector_data[sector_key]
    credits = sector_carbon_results[sector_key]['credits']
    co2 = sector_carbon_results[sector_key]['co2_tons']
    config = SECTORS[sector_key]

    gauge = go.Figure(go.Indicator(
        mode = "gauge+number+delta",
        value = credits,
        title = {'text': "Annual Carbon Credits (₹)"},
        ...
        gauge = {'axis': {'range': [0, max(credits * 1.2, 100000)]},
                 ...
                 'threshold': {'line': {'color': "red", 'width': 4}, 'value': credits}}
    ))
    gauge.update_layout(height=400)

    last_14 = df_sector.iloc[-336:]
    weekly = go.Figure()
...
# --- Analytics Callbacks (from 3D dashboard) ---
@app.callback(
...
def update_scatter(start_date, end_date):
    ...
    filtered = df_3d[(df_3d.index >= start_date) & (df_3d.index <= end_date)]
    fig = px.scatter(
...
def update_surface(start_date, end_date):
    ...
    fig = px.bar(
        hourly_day, x='hour', y='Energy_kW', color='day_name', barmode='group',
        title='Average Energy by Hour and Day (Bar Graph)',
        category_orders={'day_name': day_order}, color_discrete_sequence=px.colors.qualitative.Safe
    )
...
def update_bar(start_date, end_date):
    ...
    fig = px.bar(
        hourly_credit, x='hour', y='CarbonCredit_USD', color='CarbonCredit_USD',
        title='Average Carbon Credit by Hour', color_continuous_scale='Greens'
    )
    return fig
...
def live_risk(_, __):
    latest = df_3d.iloc[-1]
    return html.Div([
        dcc.Graph(figure=go.Figure(go.Indicator(mode="gauge+number", value=latest['PowerCut_Risk'] * 100, title={'text': "Power Cut Risk (%)"}, gauge={'axis': {'range': [0, 100]}, 'bar': {'color': "darkred"}})), style={'display': 'inline-block', 'width': '30%'}),
        dcc.Graph(figure=go.Figure(go.Indicator(mode="gauge+number", value=latest['ShortCircuit_Risk'] * 100, title={'text': "Short Circuit Risk (%)"}, gauge={'axis': {'range': [0, 100]}, 'bar': {'color': "orange"}})), style={'display': 'inline-block', 'width': '30%'}),
        dcc.Graph(figure=go.Figure(go.Indicator(mode="gauge+number", value=latest['ExcessElectricity_Risk'] * 100, title={'text': "Excess Electricity Risk (%)"}, gauge={'axis': {'range': [0, 100]}, 'bar': {'color': "blue"}})), style={'display': 'inline-block', 'width': '30%'}),
        html.P(f"Latest hour: {latest.name} | Energy: {latest['Energy_kW']} kW | Carbon Credit: ${latest['CarbonCredit_USD']}")
    ])

if __name__ == '__main__':
    # Fix port conflicts by running on port 8052
